...
@Suite(.serialized) struct AIConfigAPITests {
    @Test func providersAreManagedThroughTheAPIAndPersistedToConfigJSON() async throws {
        try await withServer { app, home in
            ...
            let (_, renamed) = try await app.json(.PUT, "/api/settings/ai/providers/claude", body: ["kind": "anthropic", "name": "Claude Sonnet", "model": "claude-sonnet-5"])
            let claude = try #require((renamed as? [String: Any])?["providers"] as? [[String: Any]]).first { $0["id"] as? String == "claude" }
            #expect(claude?["has_api_key"] as? Bool == true, "omitting api_key keeps the stored key")

            let (_, cleared) = try await app.json(.PUT, "/api/settings/ai/providers/claude", body: ["kind": "anthropic", "name": "Claude", "model": "m", "api_key": ""])
            let clearedClaude = try #require((cleared as? [String: Any])?["providers"] as? [[String: Any]]).first { $0["id"] as? String == "claude" }
    ...
    @Test func validationAndNotFoundUseTheErrorEnvelope() async throws {
        try await withServer { app, _ in
            let (bad, err) = try await app.json(.PUT, "/api/settings/ai/providers/Bad-Id", body: ["kind": "ollama", "name": "x", "model": "m"])
            #expect(bad == .badRequest)
            #expect((err as? [String: Any])?["code"] as? String == "VALIDATION_FAILED")
            #expect(try await app.json(.PUT, "/api/settings/ai/providers/x", body: ["kind": "magic", "name": "x", "model": "m"]).0 == .badRequest)
            #expect(try await app.json(.DELETE, "/api/settings/ai/providers/ghost").0 == .notFound)
            #expect(try await app.json(.PUT, "/api/settings/ai/default", body: ["provider_id": "ghost"]).0 == .notFound)
        }
    }

    @Test func handEditedYAMLConfigIsUsedWhenPresent() async throws {
        ...
        try await withApp(configure: { try await configure($0, config: ServerConfig(home: home)) }) { app in
            ...
            #expect(((body as? [String: Any])?["providers"] as? [[String: Any]])?.first?["name"] as? String == "Local")
            _ = try await app.testing().json(.PUT, "/api/settings/ai/providers/local", body: ["kind": "ollama", "name": "Renamed", "model": "llama3.2"])
