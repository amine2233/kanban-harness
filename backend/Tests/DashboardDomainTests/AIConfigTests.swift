...
@Suite struct AIConfigTests {
    func claude(key: String? = "sk-test") throws -> AIProviderConfig {
        try AIProviderConfig(id: "claude", kind: .anthropic, name: " Claude ", model: "claude-sonnet-5", apiKey: key)
    }

    @Test func providerConfigValidatesAndTrims() throws {
        let provider = try claude()
        #expect(provider.name == "Claude")
        #expect(provider.hasAPIKey)
        #expect(try AIProviderConfig(id: "local", kind: .ollama, name: "Ollama", model: "llama3", baseURL: "http://127.0.0.1:11434").baseURL == "http://127.0.0.1:11434")
        #expect(try claude(key: "").hasAPIKey == false, "empty key is no key")
    }

    @Test func providerConfigRejectsBadInput() {
        #expect(throws: DomainError.invalidProviderId("Bad Id")) { try AIProviderConfig(id: "Bad Id", kind: .ollama, name: "x", model: "m") }
        #expect(throws: DomainError.emptyName) { try AIProviderConfig(id: "a", kind: .ollama, name: " ", model: "m") }
        #expect(throws: DomainError.emptyModel("a")) { try AIProviderConfig(id: "a", kind: .ollama, name: "x", model: "") }
        #expect(throws: DomainError.invalidOrigin("ftp://x")) { try AIProviderConfig(id: "a", kind: .ollama, name: "x", model: "m", baseURL: "ftp://x") }
    ...
    @Test func removeFallsBackToAnotherDefault() throws {
        var config = try AIConfig(providers: [claude(), AIProviderConfig(id: "local", kind: .ollama, name: "Ollama", model: "llama3")], defaultProviderId: "claude")
    ...
    @Test func setDefaultRequiresAKnownProvider() throws {
        var config = try AIConfig(providers: [claude()])
        #expect(throws: DomainError.providerNotFound("nope")) { try config.setDefault("nope") }
        #expect(throws: DomainError.providerNotFound("nope")) { try AIConfig(providers: [], defaultProviderId: "nope") }
    }
}
