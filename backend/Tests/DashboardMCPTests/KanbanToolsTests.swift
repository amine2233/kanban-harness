...
@Suite(.serialized) struct KanbanToolsTests {
    actor Workspaces {
    ...
    func text(_ result: (content: [Tool.Content], isError: Bool?)) throws -> Any {
        ...
        if let object = value as? [String: Any], let items = object["items"], object.keys.sorted() == ["count", "items"] { return items }
        return value
    }
    ...
    @Test func fullCardLifecycleThroughTools() async throws {
        let (client, server, _) = try await connectedClient()

        let projects = try #require(try text(await client.callTool(name: "list_projects")) as? [[String: Any]])
        #expect(projects.map { $0["name"] as? String } == ["Demo"])

        let boards = try #require(try text(await client.callTool(name: "list_boards", arguments: ["project": "demo"])) as? [[String: Any]])
        #expect(boards.first?["name"] as? String == "Demo")

        let columns = try #require(try text(await client.callTool(name: "list_columns", arguments: ["project": "Demo", "board": "Demo"])) as? [[String: Any]])
        #expect(columns.map { $0["name"] as? String } == ["Backlog", "To do", "In progress", "Done"])

        let created = try #require(try text(await client.callTool(name: "create_card", arguments: [
            "project": "Demo", "board": "demo", "column": "to do", "title": "Write MCP tests", "priority": "high", "description": "via MCP",
        ])) as? [String: Any])
        #expect(created["key"] as? String == "task-1")
        #expect(created["column"] as? String == "To do")
        #expect(created["status"] as? String == "todo")

        let moved = try #require(try text(await client.callTool(name: "move_card", arguments: ["project": "Demo", "board": "Demo", "card": "task-1", "column": "Done"])) as? [String: Any])
        #expect(moved["status"] as? String == "done")
        #expect(moved["column"] as? String == "Done")

        let updated = try #require(try text(await client.callTool(name: "update_card", arguments: ["project": "Demo", "board": "Demo", "card": "1", "title": "Renamed", "points": 3, "due_date": "2026-12-24", "description": ""])) as? [String: Any])
        ...
        let done = try #require(try text(await client.callTool(name: "list_cards", arguments: ["project": "Demo", "board": "Demo", "status": "done"])) as? [[String: Any]])
        #expect(done.count == 1)
        let backlog = try #require(try text(await client.callTool(name: "list_cards", arguments: ["project": "Demo", "board": "Demo", "column": "Backlog"])) as? [[String: Any]])
        #expect(backlog.isEmpty)

        let createdId = try #require(created["id"] as? String)
        let deleted = try #require(try text(await client.callTool(name: "delete_card", arguments: ["project": "Demo", "board": "Demo", "card": .string(createdId)])) as? [String: Any])
    ...
    @Test func createBoardWithAndWithoutColumns() async throws {
        let (client, server, _) = try await connectedClient()
        let seeded = try #require(try text(await client.callTool(name: "create_board", arguments: ["project": "Demo", "name": "Roadmap"])) as? [String: Any])
        #expect(seeded["position"] as? Int == 1)
        let cols = try #require(try text(await client.callTool(name: "list_columns", arguments: ["project": "Demo", "board": "Roadmap"])) as? [[String: Any]])
        #expect(cols.count == 4)
        _ = try await client.callTool(name: "create_board", arguments: ["project": "Demo", "name": "Bare", "with_default_columns": false])
        let bare = try #require(try text(await client.callTool(name: "list_columns", arguments: ["project": "Demo", "board": "Bare"])) as? [[String: Any]])
        #expect(bare.isEmpty)
        await server.stop()
    }

    @Test func errorsAreReportedAsToolErrorsNotProtocolErrors() async throws {
        let (client, server, _) = try await connectedClient()
        ...
        let badArgs = try await client.callTool(name: "create_card", arguments: ["project": "Demo", "board": "Demo"])
        #expect(badArgs.isError == true)
        let badPriority = try await client.callTool(name: "create_card", arguments: ["project": "Demo", "board": "Demo", "title": "x", "priority": "urgent"])
    ...
    @Test func cardReferencesAcceptIdNumberAndKey() throws {
        let board = UUID()
        let card = Card(boardId: board, columnId: UUID(), prefix: "KAN", cardNumber: 7, title: "t", position: 0)
