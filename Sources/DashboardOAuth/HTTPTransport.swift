...
extension HTTPTransport {
    ...
    public func postForm(_ url: URL, _ fields: [String: String], headers: [String: String] = [:]) async throws -> (status: Int, body: Data) {
        let encoded = fields.sorted { $0.key < $1.key }.map { "\($0.key)=\(Self.formEncode($0.value))" }.joined(separator: "&")
        return try await post(url, headers: headers.merging(["Content-Type": "application/x-www-form-urlencoded"]) { $1 }, body: Data(encoded.utf8))
    }

    public func postJSON(_ url: URL, _ object: [String: Any], headers: [String: String] = [:]) async throws -> (status: Int, body: Data) {
        let body = try JSONSerialization.data(withJSONObject: object, options: [.sortedKeys])
        return try await post(url, headers: headers.merging(["Content-Type": "application/json"]) { $1 }, body: body)
    }
...
public enum TokenResponse {
    public static func credential(from data: Data, status: Int, now: Date = .timestamp()) throws -> Credential {
