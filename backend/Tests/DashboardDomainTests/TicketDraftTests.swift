...
@Suite struct TicketDraftTests {
    ...
    @Test func subtasksAreParsedValidatedAndCapped() throws {
        let draft = try TicketDraft.parse(Data(#"{"title":"Epic","priority":"high","subtasks":[{"title":" One ","points":3},{"title":"Two","description":"d"}]}"#.utf8))
        #expect(draft.subtasks.map(\.title) == ["One", "Two"])
        #expect(draft.subtasks[0].points == 3 && draft.subtasks[1].description == "d")
        #expect(draft.subtasks.map(\.spec) == [SubtaskSpec(title: "One", points: 3), SubtaskSpec(title: "Two", description: "d")])
        #expect(try TicketDraft.parse(Data(#"{"title":"t"}"#.utf8)).subtasks.isEmpty)
        #expect(throws: DomainError.emptyTitle) { try TicketDraft.parse(Data(#"{"title":"t","subtasks":[{"title":""}]}"#.utf8)) }
        #expect(throws: DomainError.invalidPoints(999)) { try TicketDraft.parse(Data(#"{"title":"t","subtasks":[{"title":"x","points":999}]}"#.utf8)) }
        ...
        let partial = PartialTicketDraft.parse(Data(#"{"title":"Epic","subtasks":[{"title":"One"},{"points":2},{"title":""}]}"#.utf8))
        #expect(partial.subtasks == [PartialTicketDraft.PartialSubtask(title: "One")], "only sub-tasks with a title so far")
