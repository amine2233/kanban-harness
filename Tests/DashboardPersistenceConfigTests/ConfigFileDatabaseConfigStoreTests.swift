...
@Suite struct ConfigFileDatabaseConfigStoreTests {
    ...
    @Test func aMissingFileFallsBackToTheDefault() async throws {
        let store = ConfigFileDatabaseConfigStore(path: try path("config.yaml"), environment: [:])
    ...
    func readsTheThreadPoolSizeInBothFormats(file: String, contents: String) async throws {
        let store = ConfigFileDatabaseConfigStore(path: try path(file), environment: [:])
        try contents.write(toFile: store.path, atomically: true, encoding: .utf8)
        #expect(try await store.load().threadPoolSize == 4)
    }

    @Test func leavesOtherSectionsAlone() async throws {
        let store = ConfigFileDatabaseConfigStore(path: try path("config.yaml"), environment: [:])
        try """
        ai:
        ...
        #expect(try await ConfigFileAIConfigStore(path: store.path, environment: [:]).load().defaultProviderId == "claude")
    }

    @Test func theEnvironmentOverridesTheFile() async throws {
        let store = ConfigFileDatabaseConfigStore(
            path: try path("config.yaml"),
