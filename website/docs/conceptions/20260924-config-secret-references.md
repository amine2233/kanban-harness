---
slug: /conceptions/config-secret-references
title: config secret references
---

The companion to [`oauth-token-store.md`](oauth-token-store), and the other half of the split
...
[`daemon.md`](../daemon) already documents people arriving at this from the
...
It is also the syntax [`T-20`](https://github.com/amine2233/kanban-harness/blob/main/docs/PRD.md) already plans for `mcp.json` in Claude Code's format,
...
**Decision.** [R-23](https://github.com/amine2233/kanban-harness/blob/main/docs/RULES.md) covers logging. Two more, because referencing invites them:
...
([`cli-as-source-of-truth.md`](https://github.com/amine2233/kanban-harness/blob/main/docs/conceptions/20260923-cli-as-source-of-truth.md)), which puts the home **inside the work
tree**. Everything this note and
[`oauth-token-store.md`](oauth-token-store) put in the home — `.env`, `credentials.json`, the
...
- **OAuth tokens** — [`oauth-token-store.md`](oauth-token-store). They cannot be referenced;
