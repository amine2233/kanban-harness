...
public struct RemoteProjectCommands: ProjectCommands {
    ...
    public func add(name: String, path: String, storage: StorageKind?) async throws(ServiceError) -> Project {
        try await client.send(
            "POST",
            "api/projects",
            body: CreateProjectRequest(name: name, path: path, storage: storage),
            as: Project.self
        )
    }
    ...
    public func changeStorage(
        ...
        let project = try await get(reference)
        return try await client.send(
            "PATCH", "api/projects/\(project.id.uuidString)", body: UpdateProjectRequest(storage: storage),
            as: Project.self
        )
    }
    ...
    public func boards(_ reference: ProjectRef) async throws(ServiceError) -> [BoardSummary] {
        ...
        let page = try await client.send(
            "GET", "api/projects/\(project.id.uuidString)/kanban/v1/boards?page_size=500", body: Empty?.none,
            as: Page<BoardResponse>.self
        )
...
public struct RemoteSettingsCommands: SettingsCommands {
    ...
    public func update(
        ...
        try await client.send(
            "PATCH", "api/settings",
            body: UpdateSettingsRequest(defaultStorage: defaultStorage, corsOrigins: corsOrigins),
            as: Settings.self
        )
    }
}
...
public struct RemoteAIConfigCommands: AIConfigCommands {
    ...
    public func current() async throws(ServiceError) -> AIConfig {
        try await Self.config(client.send("GET", "api/settings/ai", body: Empty?.none, as: AIConfigDTO.self))
    }
    ...
    public func upsert(_ provider: AIProviderConfig) async throws(ServiceError) -> AIConfig {
        let body = UpsertAIProviderRequest(
            ...
            pricing: provider.pricing, oauth: provider.oauth
        )
        return try await Self.config(client.send(
            "PUT",
    ...
    public func remove(_ id: String) async throws(ServiceError) -> AIConfig {
        try await Self.config(client.send(
            "DELETE",
    ...
    public func setDefault(_ id: String) async throws(ServiceError) -> AIConfig {
        try await Self.config(client.send(
            ...
            body: SetDefaultAIProviderRequest(providerId: id),
            as: AIConfigDTO.self
        ))
    }
...
public struct RemoteSignInCommands: SignInCommands {
    ...
    public func begin(providerId: String, callback: URL) async throws(ServiceError) -> URL {
        let response = try await client.send(
            "POST",
            "api/settings/ai/providers/\(providerId)/sign-in",
            body: Empty?.none,
            as: SignInResponse.self
        )
        guard let url = URL(string: response.url) else { throw .remote(
            code: "BAD_RESPONSE",
            message: "sign-in URL is not a URL"
        ) }
    ...
    public func signOut(providerId: String) async throws(ServiceError) {
        _ = try await client.send(
            "DELETE",
            "api/settings/ai/providers/\(providerId)/credential",
            body: Empty?.none,
            as: AIConfigDTO.self
        )
    }
...
public struct RemoteBoardCommands: BoardCommands {
    ...
    public init(client: DashboardClient) {
        self.client = client
        self.projects = RemoteProjectCommands(client: client)
    }

    private func base(_ project: ProjectRef) async throws(ServiceError) -> String {
        try await "api/projects/\(projects.get(project).id.uuidString)/kanban/v1"
    }

    public func boards(_ project: ProjectRef) async throws(ServiceError) -> [Board] {
        let page = try await client.send(
            "GET",
            "\(base(project))/boards?page_size=500",
            body: Empty?.none,
            as: Page<BoardResponse>.self
        )
    ...
    public func createBoard(
        _ project: ProjectRef,
        name: String,
        withDefaultColumns: Bool
    ) async throws(ServiceError) -> Board {
        let body = CreateBoardRequest(name: name, withDefaultColumns: withDefaultColumns)
        return try await Self.board(client.send(
            "POST",
            "\(base(project))/boards",
            body: body,
            as: BoardResponse.self
        ))
    }
    ...
    public func columns(_ project: ProjectRef, boardId: UUID) async throws(ServiceError) -> [Column] {
        let page = try await client.send(
            "GET",
            "\(base(project))/boards/\(boardId.uuidString)/columns?page_size=500",
            body: Empty?.none,
            as: Page<ColumnResponse>.self
        )
    ...
    public func cards(_ project: ProjectRef, boardId: UUID) async throws(ServiceError) -> [Card] {
        let page = try await client.send(
            "GET",
            "\(base(project))/boards/\(boardId.uuidString)/cards?page_size=500",
            body: Empty?.none,
            as: Page<CardResponse>.self
        )
    ...
    ) async throws(ServiceError) -> Card {
        let body = CreateCardRequest(
            title: title,
            description: description,
            priority: PriorityDTO(priority),
            aiCost: aiCost,
            subtasks: subtasks.isEmpty ? nil : subtasks.map(SubtaskRequest.init)
        )
        let response = try await client.send(
            "POST",
            "\(base(project))/columns/\(columnId.uuidString)/cards",
            body: body,
            as: CardResponse.self
        )
        guard let card = Self.card(response) else { throw .remote(
            code: "BAD_RESPONSE",
            message: "unknown card enum values"
        ) }
    ...
    public func children(
        ...
        let page = try await client.send(
            "GET",
            "\(base(project))/boards/\(boardId.uuidString)/cards/\(cardId.uuidString)/children?page_size=500",
            body: Empty?.none,
            as: Page<CardResponse>.self
        )
        return page.items.compactMap(Self.card)
    ...
    public func setParent(
        ...
        let response = try await client.send(
            "PUT",
            "\(base(project))/boards/\(boardId.uuidString)/cards/\(cardId.uuidString)/parent",
    ...
    public func updateCard(
        ...
        let response = try await client.send(
            "PATCH",
            "\(base(project))/boards/\(boardId.uuidString)/cards/\(cardId.uuidString)",
            body: body,
            as: CardResponse.self
        )
        guard let card = Self.card(response) else { throw .remote(
    ...
    public func deleteCard(
        ...
        try await client.send(
            "DELETE",
            "\(base(project))/boards/\(boardId.uuidString)/cards/\(cardId.uuidString)\(query)",
            body: Empty?.none
        )
    }
        ...
        AsyncThrowingStream { continuation in
            let task = Task {
                do {
                    ...
                    for try await line in client.eventStream(
                        "POST",
                        "api/projects/\(id.uuidString)/ai/tickets/draft",
                        body: request
                    ) {
                        guard let (event, data) = parser.feed(line: line),
                              let frame = try AssistantFrame.decode(
                                  event: event,
                                  data: data
                              ) else { continue }
                        ...
                        case let .stage(stage):
                            ...
                            continuation.yield(.stage(AssistantStage(
                                step,
                                detail: stage.detail,
                        ...
                        case let .result(response):
                            continuation.yield(.result(DraftedTicket(
                                draft: response.draft,
                                providerId: response.provider,
                                model: response.model,
                                usage: Self.usage(response.usage)
                            )))
                        case let .error(apiError): throw ServiceError.remote(
                                code: apiError.code,
