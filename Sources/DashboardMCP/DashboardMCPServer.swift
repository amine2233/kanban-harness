...
public enum DashboardMCPServer {
    ...
    public static func make(
        projects: any ProjectCommands,
        boards: any BoardCommands
    ) async -> Server {
        let server = Server(
            name: name,
            version: version,
            instructions: "Kanban boards for registered projects. Refer to projects, boards and columns "
                + "by name or id; cards by id, number or key (task-12).",
        ...
        await server.withMethodHandler(ListTools.self) {
            _ in .init(tools: KanbanTools.all)
        }
