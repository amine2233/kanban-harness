...
enum KanbanTools {
    ...
    static let all: [Tool] = [
        Tool(
            name: "list_projects",
            description: "List registered projects (folders holding a kanban workspace).",
            inputSchema: schema([:], required: [])
        ),
        Tool(
            name: "list_boards",
            description: "List the boards of a project.",
            inputSchema: schema(["project": project], required: ["project"])
        ),
        Tool(
            name: "create_board",
            ...
            inputSchema: schema(
                ["project": project, "name": prop("string", "Board name"), "with_default_columns": prop(
                    "boolean",
                    "Seed the default columns (default true)"
                )],
        ...
        Tool(
            name: "list_columns",
            description: "List the columns of a board in position order, with WIP limits and default statuses.",
            inputSchema: schema(["project": project, "board": board], required: ["project", "board"])
        ),
        Tool(
            name: "list_cards",
            description: "List the cards of a board, optionally filtered by column and/or status.",
            inputSchema: schema([
        ...
        Tool(
            name: "move_card",
            description: "Move a card to another column of the same board (its status follows the column's rules).",
            inputSchema: schema(
                [
                    "project": project,
                    "board": board,
                    "card": prop("string", "Card id, number or key"),
                    "column": prop(
                        "string",
                        "Destination column name or id"
                    )
                ],
                required: ["project", "board", "card", "column"]
            )
        ),
        Tool(
            name: "delete_card",
            description: "Delete a card permanently. Its sub-tasks are kept (detached) unless with_children is true.",
            inputSchema: schema(
                [
                    "project": project,
                    "board": board,
                    ...
                    "with_children": prop(
                        "boolean",
                        "Also delete the card's sub-tasks (default false)"
                    )
                ],
                required: ["project", "board", "card"]
            )
        ...
        Tool(
            ...
            description: "Break a card down: create child cards in the parent's column, linked to it.",
            inputSchema: schema([
                ...
                "subtasks": .object([
                    ...
                    "items": schema([
                        "title": prop("string", "Title"), "description": prop(
                            "string",
                            "Description (markdown)"
                        ),
                        "priority": prop(
                            "string",
                            "Priority (default: the parent's)",
        ...
        Tool(
            ...
            description: "Make a card a sub-task of another card on the same board (one parent per card, no cycles).",
            inputSchema: schema(
                [
                    "project": project,
                    "board": board,
                    "card": prop("string", "Child card id, number or key"),
                    "parent": prop(
                        "string",
                        "Parent card id, number or key"
                    )
                ],
                required: ["project", "board", "card", "parent"]
            )
        ),
        Tool(
            name: "remove_card_parent", description: "Detach a sub-task from its parent.",
            inputSchema: schema(
                ["project": project, "board": board, "card": prop("string", "Child card id, number or key")],
                required: ["project", "board", "card"]
            )
        )
    ]
...
struct KanbanToolDispatcher: Sendable {
    ...
    private func dispatch(_ name: String, _ args: Arguments) async throws -> Value {
        switch name {
        case "list_projects":
            return try await Self.items(projects.list().map(ProjectView.init))
        case "list_boards":
            return try await Self.items(boards.boards(args.projectRef()).map(BoardView.init))
        case "create_board":
            let board = try await boards.createBoard(
                args.projectRef(),
                name: args.string("name"),
                withDefaultColumns: args.bool("with_default_columns") ?? true
            )
            return try Value(BoardView(board))
        case "list_columns":
            let (project, board) = try await resolveBoard(args)
            return try await Self.items(boards.columns(project, boardId: board.id).map(ColumnView.init))
        case "list_cards":
            let (project, board) = try await resolveBoard(args)
            ...
            if let status = args.optionalString("status") {
                guard let wanted = CardStatus(wireValue: status)
                else { throw ToolError("unknown status '\(status)'") }
        ...
        case "create_card":
            ...
            if let filter = args.optionalString("column") {
                ...
                else { throw ToolError("board '\(board.name)' has no columns") }

                column = first
            }
            let priority = try args.optionalString("priority").map { raw -> CardPriority in
                guard let value = CardPriority(wireValue: raw)
                else { throw ToolError("unknown priority '\(raw)'") }
        ...
        case "update_card":
            let (project, board) = try await resolveBoard(args)
            let columns = try await boards.columns(project, boardId: board.id)
            let card = try await Self.findCard(boards.cards(project, boardId: board.id), args.string("card"))
            var changes = CardChanges(title: args.optionalString("title"))
            if let description = args
                .optionalString("description") {
                changes.description = .some(description.isEmpty ? nil : description)
            }
            ...
            if let raw = args.optionalString("status") {
                guard let value = CardStatus(wireValue: raw)
                else { throw ToolError("unknown status '\(raw)'") }

                changes.status = value
            }
            if let points = args.int("points") { changes.points = .some(points == 0 ? nil : points) }
            if let raw = args.optionalString("due_date") { changes.dueDate = try .some(Self.parseDate(raw)) }
            let updated = try await boards.updateCard(
                project,
                boardId: board.id,
                cardId: card.id,
                changes: changes
        ...
        case "move_card":
            ...
            let columns = try await boards.columns(project, boardId: board.id)
            let card = try await Self.findCard(boards.cards(project, boardId: board.id), args.string("card"))
            let column = try Self.find(columns, args.string("column"), what: "column")
            let moved = try await boards.updateCard(
                project,
                boardId: board.id,
                cardId: card.id,
                changes: CardChanges(columnId: column.id)
            )
            return try Value(CardView(moved, columns: columns))
        case "delete_card":
            let (project, board) = try await resolveBoard(args)
            let card = try await Self.findCard(boards.cards(project, boardId: board.id), args.string("card"))
            try await boards.deleteCard(
                project,
                boardId: board.id,
                cardId: card.id,
                includingChildren: args.bool("with_children") ?? false
            )
            return .object([
                "deleted": .string(card.id.uuidString.lowercased()),
                "key": .string("\(card.prefix)-\(card.cardNumber)")
            ])
        case "create_subtasks":
            let (project, board) = try await resolveBoard(args)
            let columns = try await boards.columns(project, boardId: board.id)
            let parent = try await Self.findCard(
                boards.cards(project, boardId: board.id),
                args.string("card")
            )
            guard case let .array(items)? = args.values["subtasks"]
            else { throw ToolError("subtasks must be an array") }
            ...
            for spec in specs {
                let child = try await boards.createCard(
                    project,
                ...
                if let points = spec.points {
                    _ = try await boards.updateCard(
                        project,
                        boardId: board.id,
                        cardId: child.id,
                        changes: CardChanges(points: .some(points))
                    )
                }
                _ = try await boards.setParent(
                    project,
                    boardId: board.id,
                    cardId: child.id,
                    parentId: parent.id
            ...
            return try await Self
                .items(boards.children(project, boardId: board.id, cardId: parent.id).map { CardView(
                    $0,
                    columns: columns
                ) })
        ...
        case "set_card_parent", "remove_card_parent":
            ...
            let child = try Self.findCard(cards, args.string("card"))
            let parent = try name == "set_card_parent" ? try Self.findCard(cards, args.string("parent")) : nil
            let updated = try await boards.setParent(
                project,
                boardId: board.id,
                cardId: child.id,
                parentId: parent?.id
    ...
    private func resolveBoard(_ args: Arguments) async throws -> (ProjectRef, Board) {
        ...
        let board = try await Self.find(boards.boards(project), args.string("board"), what: "board")
        return (project, board)
    }

    static func find<T: Identifiable>(_ items: [T], _ reference: String, what: String) throws -> T
        where T.ID == UUID {
        if let id = UUID(uuidString: reference),
           let match = items.first(where: { $0.id == id }) { return match }
    ...
    static func findCard(_ cards: [Card], _ reference: String) throws -> Card {
        if let id = UUID(uuidString: reference),
           let match = cards.first(where: { $0.id == id }) { return match }
        ...
        if let number,
           ...
           .first(where: {
               $0
                   .cardNumber == number &&
                   (reference.contains("-") ? reference.lowercased().hasPrefix($0.prefix.lowercased()) : true)
           }) {
            return match
        }
    ...
    static func items<T: Codable>(_ list: [T]) throws -> Value {
        try .object(["items": Value(list), "count": .int(list.count)])
    }
...
struct Arguments {
    ...
    func string(_ key: String) throws -> String {
        guard let value = optionalString(key),
              !value.isEmpty else { throw ToolError("missing argument '\(key)'") }
    ...
    func projectRef() throws -> ProjectRef {
        try .parse(string("project"))
    }
}
