...
actor FakeSignIn: ProviderSignIn {
    ...
    init(expiresIn: TimeInterval? = nil) { self.expiresIn = expiresIn }

    nonisolated func authorizationURL(callback: URL, state: String, codeChallenge: String) -> URL {
        URL(string: "https://vendor.test/auth?state=\(state)&challenge=\(codeChallenge)&cb=\(callback.absoluteString)")!
    }
    ...
    func exchange(code: String, codeVerifier: String, callback: URL) async throws -> Credential {
        ...
        return Credential(secret: "token-for-\(code)", refreshToken: "r", expiresAt: expiresIn.map { Date.timestamp().addingTimeInterval($0) })
    }

    func refresh(_ credential: Credential) async throws -> Credential? {
        refreshed += 1
        return Credential(secret: "renewed", refreshToken: credential.refreshToken, expiresAt: .timestamp().addingTimeInterval(3600))
    }
}

@Suite struct ProviderSignInServiceTests {
    let callback = URL(string: "http://127.0.0.1:5175/api/auth/callback")!

    func fixture(_ vendor: FakeSignIn = FakeSignIn()) async throws -> (ProviderSignInService, AIConfigService, InMemoryCredentialStore, FakeSignIn) {
        let credentials = InMemoryCredentialStore()
        let aiConfig = AIConfigService(store: InMemoryAIConfigStore())
        _ = try await aiConfig.upsert(try AIProviderConfig(id: "router", kind: .openrouter, name: "Router", model: "m"))
        _ = try await aiConfig.upsert(try AIProviderConfig(id: "claude", kind: .anthropic, name: "Claude", model: "m", apiKey: "sk"))
        var registry = AIProviderRegistry()
        registry.registerSignIn(.openrouter) { _ in vendor }
        return (ProviderSignInService(aiConfig: aiConfig, credentials: credentials, registry: registry), aiConfig, credentials, vendor)
    }

    @Test func beginThenCompleteStoresTheCredential() async throws {
        ...
        #expect(PKCE.challenge(for: call.verifier) == query["challenge"], "the verifier matches the challenge sent to the vendor")
    }

    @Test func unknownOrReusedStateIsRefused() async throws {
    ...
    @Test func kindsWithoutSignInAndUnknownProvidersFail() async throws {
        let (service, _, _, _) = try await fixture()
        await #expect(throws: ServiceError.self) { _ = try await service.begin(providerId: "claude", callback: callback) }
        await #expect(throws: ServiceError.self) { _ = try await service.begin(providerId: "ghost", callback: callback) }
    }

    @Test func signOutForgetsTheCredential() async throws {
    ...
    @Test func expiringTokensAreRenewedBeforeUse() async throws {
        let (service, _, credentials, vendor) = try await fixture()
        let config = try AIProviderConfig(id: "router", kind: .openrouter, name: "Router", model: "m", apiKey: "old")
        try await credentials.set(Credential(secret: "old", refreshToken: "r", expiresAt: .timestamp().addingTimeInterval(3600)), for: "router")
        #expect(try await service.refreshed(config).apiKey == "old", "a fresh token is left alone")

        try await credentials.set(Credential(secret: "old", refreshToken: "r", expiresAt: .timestamp().addingTimeInterval(10)), for: "router")
        ...
        let plain = try AIProviderConfig(id: "claude", kind: .anthropic, name: "Claude", model: "m", apiKey: "sk")
