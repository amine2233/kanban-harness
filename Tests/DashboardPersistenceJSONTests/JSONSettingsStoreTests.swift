...
struct JSONSettingsStoreTests {
    ...
    @Test
    func handEditedFileIsPickedUpOnNextLoad() async throws {
        let store = try store()
        try await store.save(.default)
        try AtomicFile.write(
            Data(#"{"default_storage": "sqlite", "cors_origins": ["HTTP://Localhost:5173/"]}"#.utf8),
            to: store.path
        )
    ...
    @Test
    func invalidFileSurfacesAsCorrupt() async throws {
        let store = try store()
