...
struct JSONCompleterTests {
    ...
    @Test
    func everyPrefixParsesAndNeverInventsAValue() throws {
            ...
            for (key, value) in parsed {
                ...
                if let s = value as? String, let e = expected as? String {
                    ...
                    for (i, item) in list.enumerated() {
                        #expect(
                            i == list.count - 1 ? e[i].hasPrefix(item) : e[i] == item,
                            "prefix \(cut): \(key)[\(i)]"
                        )
                    }
    ...
    @Test
    func completesTheHalfTypedShapes() {
        ...
        for (input, expected) in cases {
            let got = JSONCompleter.complete(input)
    ...
    @Test
    func partialDraftParsesLenientlyAndReportsEmptiness() {
        #expect(PartialTicketDraft.parse(Data()).isEmpty)
        let partial = PartialTicketDraft
            .parse(Data(#"{"title":"T","priority":"bogus","acceptance_criteria":["a",1],"points":"3"}"#.utf8))
