...
struct KanbanManagementTests {
    ...
    @Test
    func createRenameReorderAndDeleteBoards() async throws {
        try await withServer { app, home in
            let base = try await project(app, home)
            let (status, created) = try await app.json(
                .POST,
                "\(base)/boards",
                body: ["name": "Second", "card_prefix": "SEC"]
            )
            ...
            let (_, bare) = try await app.json(
                .POST,
                "\(base)/boards",
                body: ["name": "Bare", "with_default_columns": false]
            )
            ...
            let (renamed, body) = try await app.json(
                .PATCH,
                "\(base)/boards/\(secondId)",
                body: ["name": "Renamed", "position": 0, "description": "desc"]
            )
            #expect(renamed == .ok)
            ...
            let names = ((list as? [String: Any])?["items"] as? [[String: Any]])?
            ...
            #expect(try await app.json(.PATCH, "\(base)/boards/\(secondId)", body: ["name": " "])
                .0 == .badRequest)
    ...
    @Test
    func cloneBoardCopiesColumnsAndCards() async throws {
        try await withServer { app, home in
            let base = try await project(app, home)
            ...
            _ = try await app.json(
                .POST,
                "\(base)/columns/\(doing)/cards",
                body: ["title": "Original", "priority": "critical"]
            )

            let (status, clone) = try await app.json(
                .POST,
                "\(base)/boards/\(boardId)/clone",
                body: ["name": "Copy"]
            )
            #expect(status == .created)
    ...
    @Test
    func columnLifecycleCreateUpdateReorderDelete() async throws {
        try await withServer { app, home in
            let base = try await project(app, home)
            let (boardId, columns) = try await app.firstBoardAndColumns(base)
            let (status, created) = try await app.json(
                ...
                body: ["name": "Review", "wip_limit": 2, "default_status": "blocked"]
            )
            #expect(status == .created)
            ...
            let (updated, body) = try await app.json(
                .PATCH,
                "\(base)/boards/\(boardId)/columns/\(reviewId)",
                body: ["name": "QA", "position": 1, "wip_limit": NSNull(), "default_status": "in_progress"]
            )
            #expect(updated == .ok)
            ...
            #expect(try await app.json(
                .POST,
                "\(base)/boards/\(boardId)/columns",
                body: ["name": "x", "default_status": "weird"]
            ).0 == .badRequest)
            #expect(try await app.json(.DELETE, "\(base)/boards/\(boardId)/columns/\(UUID().uuidString)")
                .0 == .notFound)
        }
    }

    @Test
    func moveCardToAnotherBoardAndPatchExtraFields() async throws {
        try await withServer { app, home in
            let base = try await project(app, home)
            let (boardId, columns) = try await app.firstBoardAndColumns(base)
            let todo = try #require(columns[0]["id"] as? String)
            let (_, card) = try await app.json(
                .POST,
                "\(base)/columns/\(todo)/cards",
                body: ["title": "Traveller"]
            )
            let cardId = try #require((card as? [String: Any])?["id"] as? String)
            let (_, other) = try await app.json(
                .POST,
                "\(base)/boards",
                body: ["name": "Other", "card_prefix": "OTH"]
            )
            let otherId = try #require((other as? [String: Any])?["id"] as? String)

            let (status, moved) = try await app.json(
                .PATCH,
                "\(base)/boards/\(boardId)/cards/\(cardId)",
                body: ["board_id": otherId, "points": 3, "due_date": "2026-12-01T00:00:00Z"]
            )
            #expect(status == .ok)
            let body = try #require(moved as? [String: Any])
            #expect(body["board_id"] as? String == otherId
                .lowercased() || body["board_id"] as? String == otherId)
            ...
            #expect(try await app.json(.GET, "\(base)/boards/\(boardId)/cards").1
                .flatMap { ($0 as? [String: Any])?["total"] as? Int } == 0)
            let (_, otherCards) = try await app.json(.GET, "\(base)/boards/\(otherId)/cards")
            #expect((otherCards as? [String: Any])?["total"] as? Int == 1)
            #expect(
                try await app
                    .json(.PATCH, "\(base)/boards/\(boardId)/cards/\(cardId)", body: ["title": "gone"])
                    .0 == .notFound,
                "card no longer belongs to the old board"
            )
        }
    }
}
