...
struct SignInAPITests {
    @Test
    func signInRoutesRoundTripThroughTheServer() async throws {
        try await withServer { app, home in
            _ = try await app.json(
                .PUT,
                "/api/settings/ai/providers/router",
                body: ["kind": "openrouter", "name": "Router", "model": "m"]
            )
            ...
            let callbackParam = try #require(URLComponents(url: url, resolvingAgainstBaseURL: false)?
            ...
            let state = try #require(URLComponents(url: callback, resolvingAgainstBaseURL: false)?.queryItems?
            ...
            let denied = try await app.sendRequest(
                .GET,
                "/api/auth/callback?state=\(state)&error=access_denied"
            )
            ...
            let (signedOut, after) = try await app.json(
                .DELETE,
                "/api/settings/ai/providers/router/credential"
            )
            #expect(signedOut == .ok)
            #expect(((after as? [String: Any])?["providers"] as? [[String: Any]])?
                .first?["has_api_key"] as? Bool == false)
    ...
    @Test
    func oauthAppSettingsAreStoredAndOnlyTheClientIdIsExposed() async throws {
        try await withServer { app, home in
