...
@Suite(.serialized) struct MCPHTTPTests {
    func rpc(_ app: TestingApplicationTester, _ body: [String: Any]) async throws -> (HTTPStatus, [String: Any]) {
        var headers = HTTPHeaders()
        headers.contentType = .json
        headers.add(name: .accept, value: "application/json")
        headers.add(name: "MCP-Protocol-Version", value: "2025-06-18")
        let response = try await app.sendRequest(.POST, "/mcp", headers: headers, body: ByteBuffer(data: try JSONSerialization.data(withJSONObject: body)))
        let data = Data(buffer: response.body)
        let json = data.isEmpty ? [:] : ((try JSONSerialization.jsonObject(with: data) as? [String: Any]) ?? [:])
        return (response.status, json)
    }

    @Test func mcpOverHTTPListsAndCallsTools() async throws {
        try await withServer { app, home in
            let project = try await app.createProject("Demo", at: home + "/demo")
            let (initStatus, initBody) = try await rpc(app, ["jsonrpc": "2.0", "id": 1, "method": "initialize", "params": ["protocolVersion": "2025-06-18", "capabilities": [:], "clientInfo": ["name": "test", "version": "1"]]])
            #expect(initStatus == .ok)
            #expect(((initBody["result"] as? [String: Any])?["serverInfo"] as? [String: Any])?["name"] as? String == "mvp-dashboard")
            ...
            let (_, call) = try await rpc(app, ["jsonrpc": "2.0", "id": 3, "method": "tools/call", "params": ["name": "create_card", "arguments": ["project": "Demo", "board": "Demo", "title": "From MCP over HTTP"]]])
            ...
            #expect((cards as? [String: Any])?["total"] as? Int == 1, "the card is visible through the REST API too")
        }
    }

    @Test func mcpRejectsNonPostAndBadOrigins() async throws {
        try await withServer { app, _ in
            ...
            let response = try await app.sendRequest(.POST, "/mcp", headers: headers, body: ByteBuffer(string: #"{"jsonrpc":"2.0","id":1,"method":"tools/list"}"#))
