    ...
    public static func standard(
        ...
        registry.register(.apple) { config in
            AnyLanguageModelProvider(config: config) {
                ...
                throw AIProviderError
                    .unavailable("Apple's on-device model is only available on macOS 26 or later")
#endif
            }
        }
        registry.register(.anthropic) { config in
            AnyLanguageModelProvider(config: config) {
                ...
                else { throw AIProviderError.notConfigured("\(config.name) has no API key") }

                return AnthropicLanguageModel(
                    baseURL: Self.url(config.baseURL, default: "https://api.anthropic.com/v1"),
                    apiKey: key,
                    model: config.model
                )
            }
        ...
        registry.register(.openai) { config in
            AnyLanguageModelProvider(config: config) {
                OpenAILanguageModel(
                    baseURL: Self.url(config.baseURL, default: "https://api.openai.com/v1"),
        ...
        registry.register(.gemini) { config in
            AnyLanguageModelProvider(config: config) {
                ...
                return GeminiLanguageModel(
                    baseURL: Self
                        .url(config.baseURL, default: "https://generativelanguage.googleapis.com/v1beta"),
                    apiKey: key,
                    model: config.model
                )
            }
        ...
        registry.register(.ollama) { config in
            AnyLanguageModelProvider(config: config) {
                OllamaLanguageModel(
                    baseURL: Self.url(config.baseURL, default: "http://127.0.0.1:11434"),
                    model: config.model
                )
            }
        }
