...
struct RuntimeTests {
    ...
    @Test
    func runtimeConfigDefaultsToTheHomeDependency() async throws {
        ...
        #expect(RuntimeConfig(home: home, claudeExecutable: "/stub/claude")
            .claudeExecutable == "/stub/claude")
        #expect(RuntimeConfig(home: home)
            .claudeExecutable ==
            (ProcessInfo.processInfo.environment["MVP_DASHBOARD_CLAUDE_BIN"] ?? "claude"))
    }

    @Test
    func homeKeyReadsEnvironmentThenLocalThenXDGThenHome() {
        #expect(HomeKey
            .defaultHome(environment: ["MVP_DASHBOARD_HOME": "/x"], currentDirectory: "/nowhere") == "/x")
        #expect(HomeKey
            .defaultHome(environment: ["XDG_CONFIG_HOME": "/cfg"], currentDirectory: "/nowhere") ==
            "/cfg/kanban-harness")
        #expect(HomeKey
            .defaultHome(environment: ["HOME": "/me"], currentDirectory: "/nowhere") ==
            "/me/.config/kanban-harness")
    }

    /// A folder with its own `.kanban-harness` keeps its own home; a sibling without
    /// one falls back to the user's.
    @Test
    func homeKeyPrefersALocalDirectoryOverTheGlobalOne() throws {
        ...
        #expect(HomeKey
            .defaultHome(environment: ["HOME": "/me"], currentDirectory: workspace + "/..") ==
            "/me/.config/kanban-harness")
        #expect(
            HomeKey
                .defaultHome(environment: ["MVP_DASHBOARD_HOME": "/x"], currentDirectory: workspace) == "/x",
            "an explicit environment home still wins"
        )
    }

    @Test
    func registerWiresSingletonsAndServicesWork() async throws {
        let home = try tempHome()
    ...
    @Test
    func shutdownClosesResourcesAndAllowsReopening() async throws {
        let home = try tempHome()
        let container = CascadeKit.Application()
        try await DashboardRuntime.register(on: container, config: RuntimeConfig(home: home))
    ...
    @Test
    func disabledProvidersNeverRegister() async throws {
        let home = try tempHome()
        let container = CascadeKit.Application()
    ...
    @Test
    func disabledProvidersComeFromTheEnvironment() {
        #expect(RuntimeConfig.disabledFromEnvironment([:]).isEmpty)
        #expect(
            RuntimeConfig
                .disabledFromEnvironment(["MVP_DASHBOARD_PROVIDERS_DISABLED": "huggingface, claude_code"])
    ...
    @Test
    func registerAcceptsAnExternalRegistryDatabase() async throws {
        let home = try tempHome()
        let external = try SQLiteDatabase.registry(path: home + "/external.sqlite")
        try await external.migrate()
        let container = CascadeKit.Application()
        try await DashboardRuntime.register(
            on: container,
            config: RuntimeConfig(home: home),
            registryDatabase: external.database
        )
