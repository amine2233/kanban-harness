...
@Suite struct KanbanJSONStoreTests {
    ...
    func storeWithFixture() throws -> KanbanJSONStore {
        let store = try store()
        try AtomicFile.write(try fixture("kanban-v18"), to: store.path)
        return store
    }

    func rawJSON(_ store: KanbanJSONStore) throws -> [String: Any] {
        try #require(JSONSerialization.jsonObject(with: Data(contentsOf: URL(fileURLWithPath: store.path))) as? [String: Any])
    }
    ...
    @Test func loadsFileWrittenByKanbanRs() async throws {
    ...
    @Test func saveAfterLoadPreservesUnmodelledSectionsAndEnvelope() async throws {
        ...
        #expect(card["id"] as? String == (card["id"] as? String)?.lowercased(), "uuids are written lowercase like kanban-rs")
    ...
    @Test func saveOnFreshStoreWritesFullKanbanSkeleton() async throws {
        let store = try store()
        var workspace = Workspace()
        workspace.createBoardWithTemplateColumns(name: "New")
        try await store.save(workspace)
        let data = try #require(try rawJSON(store)["data"] as? [String: Any])
        #expect(Set(data.keys) == ["archived_boards", "archived_cards", "boards", "cards", "columns", "graph", "prefixes", "sprints"])
    }

    @Test func loadRefusesOtherFormatVersions() async throws {
        let store = try store()
        try AtomicFile.write(Data(#"{"version": 17, "metadata": {"instance_id": "6f1c1c1e-2b0c-4b7c-9d3a-1c2b3c4d5e6f", "saved_at": "2026-01-01T00:00:00Z"}, "data": {}}"#.utf8), to: store.path)
        do {
    ...
    @Test func loadCorruptSectionThrowsCorrupt() async throws {
        let store = try store()
        try AtomicFile.write(Data(#"{"version": 18, "metadata": {"instance_id": "6f1c1c1e-2b0c-4b7c-9d3a-1c2b3c4d5e6f", "saved_at": "2026-01-01T00:00:00Z"}, "data": {"boards": [{"id": "nope"}]}}"#.utf8), to: store.path)
